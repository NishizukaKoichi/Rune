...
cage-fe-snap-update:
	cd $(CAGE_FRONTEND_DIR) && pnpm test -- -u --watch=false

cage-fe-lint:
	cd $(CAGE_FRONTEND_DIR) && pnpm lint

cage-fe-type:
	cd $(CAGE_FRONTEND_DIR) && pnpm typecheck

cage-fe-build:
	cd $(CAGE_FRONTEND_DIR) && pnpm build

cage-be-install:
	uv run pip install -r backend/requirements.txt
